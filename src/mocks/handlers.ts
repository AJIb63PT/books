...
function validateBookFields(
  ...
  if (authorIds.length === 0)
    return fail("Выберите хотя бы одного автора", 422, "author_ids");
  if (coverRequired && !hasCover)
    return fail("Обложка обязательна", 422, "cover");
  return null;
}
...
export const handlers = [
  ...
  http.post("*/api/v1/books", async ({ request }) => {
    ...
    const invalid = validateBookFields(
      title,
      year,
      authorIds,
      true,
  ...
  http.put("*/api/v1/books/:id", async ({ request, params }) => {
    ...
    mockBooks[index] = {
      ...
      title,
      year,
      description: fd.has("description")
  ...
  http.patch("*/api/v1/books/:id", async ({ request, params }) => {
    ...
    const authorIds =
      body.author_ids ?? current.authors.map((author) => author.id);
    ...
    mockBooks[index] = {
      ...
      isbn:
        body.isbn !== undefined
          ? String(body.isbn).trim() || undefined
          : current.isbn,
      authors: authorsByIds(authorIds),
    };
    return ok(mockBooks[index]);
  ...
  http.get("*/api/v1/reports/top-authors", ({ request }) => {
    ...
    const items: TopAuthor[] = [...counts.entries()]
      .map(([authorId, count]) => ({
        ...
        author_id: authorId,
        full_name:
          mockAuthors.find((author) => author.id === authorId)?.full_name ??
          `Автор #${authorId}`,
        books_count: count,
      }))
      .sort(
