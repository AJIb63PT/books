...
async function onDelete(): Promise<void> {
  if (!book.value || !window.confirm(`Удалить книгу «${book.value.title}»?`)) return
  deleting.value = true
  errorMessage.value = null
  try {
  ...
  <div v-else-if="book" class="row g-4">
    <div class="col-md-4">
      <div class="card-img-top-wrapper rounded border bg-light">
        <img v-if="book.cover_url" :src="book.cover_url" class="book-cover" :alt="book.title" />
        <div
          v-else
    ...
    <div class="col-md-8">
      ...
      <div class="text-secondary mb-3">{{ book.year }} г., ISBN: {{ book.isbn || '—' }}</div>
