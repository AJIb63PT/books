...
async function onDelete(): Promise<void> {
  if (!author.value || !window.confirm(`Удалить автора «${author.value.full_name}»?`)) return
  deleting.value = true
  errorMessage.value = null
  try {
  ...
  <div v-else-if="author">
    <div class="d-flex justify-content-between align-items-start mb-4">
      <div>
        ...
        <div class="text-secondary">
          {{ author.books?.length ?? 0 }} {{ (author.books?.length ?? 0) === 1 ? 'книга' : 'книг' }}
          в каталоге
        </div>
      </div>
    ...
    <div v-if="author.books?.length" class="row row-cols-1 row-cols-md-2 g-3">
      <div
        ...
        :key="book.id"
        class="col d-flex justify-content-between align-items-center border rounded p-3"
      >
        <div>
          <div class="fw-semibold">{{ book.title }}</div>
          <div class="text-secondary text-secondary-small">{{ book.year }} г.</div>
        </div>
        <RouterLink
