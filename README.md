...
- **SMS-уведомления** — подписка отправляет SMS через [smspilot.ru](https://sms-pilot.ru/); по умолчанию используется тестовый ключ-эмулятор (реальная отправка не происходит).
...
В спецификации бэкенда нет endpoint'а для хранения подписок, поэтому подписка реализована на фронтенде: форма «автор + телефон» отправляет SMS-сообщение напрямую через smspilot. Тестовый ключ-эмулятор никаких реальных SMS не отправляет.
